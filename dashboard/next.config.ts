...
// root in `shared/`, ONE LEVEL ABOVE the Next.js root.
//
// 1) `outputFileTracingRoot` widens Vercel's file-tracing to the repo root so
//    the bundled function image actually includes `shared/`.
// 2) `outputFileTracingIncludes` is a belt-and-braces glob that guarantees
//    every file under `shared/` is shipped (tracing alone occasionally misses
...
const nextConfig: NextConfig = {
  ...
  webpack: (config) => {
    ...
    // The shared backend code lives at repoRoot/shared/, but its `require(...)`
    // calls (e.g. require('bcryptjs')) resolve node_modules by walking up
    // from shared/, which never reaches dashboard/node_modules. Add it
