...
Your domain covers: `src/app.js`, `src/server.js`, `src/routes/`, `src/controllers/`, `src/services/authService.js`, `src/services/campaignService.js`, `src/middlewares/`, and `src/config/index.js`.
...
- Config is accessed only via `src/config/index.js` — never `process.env` directly in other files.
...
2. Read existing files in `src/routes/`, `src/controllers/`, `src/services/`, `src/middlewares/`
