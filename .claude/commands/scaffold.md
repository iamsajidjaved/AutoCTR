...
- `model <name>` → create `src/models/<name>Model.js` with async functions using `{ sql }` from `src/models/db.js`. Export named functions, no class.
- `service <name>` → create `src/services/<name>Service.js`. Import relevant model(s). Export named async functions. No Express objects — pure logic only.
...
- `migration <description>` → create `src/migrations/NNN_<description>.sql` where NNN is the next migration number (check existing files). Write idempotent SQL using `IF NOT EXISTS` or `IF EXISTS`.
