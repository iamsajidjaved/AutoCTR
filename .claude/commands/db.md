...
Your domain covers everything in `src/models/`, `src/migrations/`, and the Neon client setup.

**Project DB conventions:**
- Client: `@neondatabase/serverless` — use `sql` tagged template for simple queries, `pool` for transactions
- All DB access goes through `src/models/db.js` — never create a new connection elsewhere
- Models export named async functions (no classes)
- Migrations live in `src/migrations/NNN_name.sql`, run via `npm run db:migrate`
...
2. Read any existing files in `src/models/` and `src/migrations/` 
